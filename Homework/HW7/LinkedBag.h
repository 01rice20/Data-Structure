...
bool LinkedBag<ItemType>::add2ndToLast(const ItemType& newEntry)
{
	...
	else
	{
		...
		Node<ItemType>* last2NodePtr = new Node<ItemType>();
		last2NodePtr->setItem(newEntry);
		if (currPtr == headPtr)
		{
			last2NodePtr->setNext(headPtr);
			headPtr = last2NodePtr;
		}
		else
		{
			last2NodePtr->setNext(currPtr);
			prevPtr->setNext(last2NodePtr);
		}
		itemCount++;
	}
